...
static int	fpr_flg_d_B1(t_fpnt_list flagi, int u_l)
{
	int	i;
...
static int	fpr_flg_d_B2(t_fpnt_list flagi, int u, int u_l)
{
	int	i;
...
static int	fpr_flg_d_B3(t_fpnt_list flagi, int u_l)
{
	int	i;

	i = 0;
	if (flagi.f_tochnost > u_l)
		i = flagi.f_shirina - u_l - (flagi.f_tochnost - u_l) - 1;
	if (flagi.f_tochnost <= u_l)
		i = flagi.f_shirina - u_l - 1;
	while (i-- > 0)
		flagi.length = ft_putchar_fd_p(1, ' ', flagi);
...
{
	flagi.length = fpr_flg_d_B1(flagi, u_l);
	if (flagi.f_tochnost == 0)
	{
		flagi.length = fpr_flg_d_B2(flagi, u, u_l);
		if (u == 0)
			return (flagi.length);
	}
	if (flagi.f_tochnost > 0)
		flagi.length = fpr_flg_d_B3(flagi, u_l);
